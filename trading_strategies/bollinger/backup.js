...
async function XMinChartMonitoringInEveryXMinutes(data)
{
    try
    {
        ...
        let stopTime = startTime + data.botCycleInMinutes * 60000;
        ...
        console.log(`Bot stopped after running for ${data.botCycleInMinutes} minutes`);

        let statsText = tradesData.map(trade => `Trade: ${trade.symbol}, ROI: ${trade.roi}%, 
        PnL: ${trade.pnl} USDT`).join('\n');
        ...
        let mailOptions = 
        {
            ...
            text: `Number of Trades: ${tradesData.length}\nTrade Summary:\n${statsText}`
        }
...
try
{
    let SCRIPT = async() => 
    {
        let data = 
        {
            ...
            botCycleInMinutes: args.botCycleInMinutes, //5
